...
def dataloader_trial(subname,fre,result_all,test_id):
    ...
    subjects_stc_dir_sub_this = op.join(subjects_stc_dir_sub, hp.processed_MEG_name, hp.stc_method \
                                        ...
                                        + str(hp.loose))
    sur_fname = op.join(subjects_stc_dir_sub_this, 'emotion_MEG_EGG_DE_PSD.pickle')
    feature = []
    label = []
    test_feature = []
    test_label = []
    with open(sur_fname, 'rb') as f:
        clu = pickle.load(f)['meg_psd']
...
def svm_leave_one_trail_out():
    ...
    for sb in range(0,num_sub):
        ...
        for run in range(2, 8):
            ...
            result_all[(run - 2)*2] = result[0, 2]
            result_all[(run - 2)*2+ 1] = result[1, 2]
        num_tr=0

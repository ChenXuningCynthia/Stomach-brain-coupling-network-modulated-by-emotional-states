...
def envelope(imf):
    ...
    half=env-np.mean(env)/2
    ang= np.angle(hilbert(half))
    return ang
...
def MI(amp,phase):
    ...
    P = np.zeros([100,n_bins])
    for i in range(n_bins):
        P[:,i] = np.mean(amp[:,bin_indices == i],axis=1)
    SP=np.sum(P,axis=1)
    P = P/SP[:,np.newaxis]  # Normalize to ensure it sums to 1
    H=0
...
for sb in range(numsub):
    ...
    time = np.zeros(4)
    time[0] = events[1, 0] / 1000 - rawfile._first_time
    time[1] = time[0] + 180
    time[2] = time[0] + 185
    time[3] = time[0] + 605
    ...
    mat_name = op.join(subjects_stc_dir_sub_this, 'rest_SBMI.pickle')
